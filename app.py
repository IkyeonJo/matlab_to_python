...
# # 제목 설정
# st.title("Matlab -> Python 변환 데모 앱")
# st.markdown("---")
...
#     """
#     주어진 신호에 대해 이산 푸리에 변환(DFT)을 수행하고 결과를 반환합니다.
    
#     Parameters:
#     signal (numpy.ndarray): 입력 신호
    
#     Returns:
#     numpy.ndarray: 푸리에 변환된 신호
#     """
...
#         st.code(dft_code, language='python')
#     with col2:
...
#     # 각 기능에 따른 사용자 입력 및 결과 표시
#     if gui_option in ['푸리에 변환', '역푸리에 변환']:
#         st.subheader(gui_option)
        
#         # 사용자 입력: 주파수, 시간 범위, 샘플 수 등
...
#         if gui_option == '푸리에 변환':
#             ax.set_title('Fourier Transform 결과')
#             ax.plot(t, np.abs(transformed_signal), label='FFT')
#         elif gui_option == '역푸리에 변환':
#             ax.set_title('Inverse Fourier Transform 결과')
#             ax.plot(t, np.real(transformed_signal), label='IFFT')
#         ax.legend()
#         st.pyplot(fig)
    
#     elif gui_option == '신호처리-내삽':
#         st.subheader("신호처리 - 내삽")
...
#         ax.set_title('신호처리 - 내삽 결과')
#         ax.legend()
#         st.pyplot(fig)
    
#     elif gui_option == '신호처리-외삽':
#         st.subheader("신호처리 - 외삽")
...
# 이 앱에 대한 피드백이나 질문이 있으시면 언제든지 연락 주세요.
# """)
...
# # 제목 설정
# st.title("Matlab to Python 변환 및 신호 처리 데모 앱")
...
#     """
#     주어진 푸리에 변환된 신호에 대해 역 이산 푸리에 변환을 수행하고 결과를 반환합니다.
    
#     Parameters:
#     transformed_signal (numpy.ndarray): 푸리에 변환된 신호
    
#     Returns:
#     numpy.ndarray: 역 푸리에 변환된 신호
#     """
...
#         fig, ax = plt.subplots()
#         ax.plot(t, signal, label='Original Signal')
#         ax.plot(x_plot, plot_data, label='Transformed Signal')
#         ax.set_title(title)  # 영어로 설정
#         ax.legend()
#         st.pyplot(fig)
...
#         ax.plot(x, y, 'o', label='Original Data')
#         ax.plot(x_new, y_new, '-', label='Interpolated Data')
#         ax.set_title('Signal Processing - Interpolation Result')  # 영어로 설정
#         ax.legend()
#         st.pyplot(fig)
...
#         # 외삽 설정
#         spline = UnivariateSpline(x, y, s=0)
#         x_extrap = np.linspace(0, 15, 150)  # 비선형 곡선을 얻기 위한 충분한 범위와 데이터 포인트 수 설정
#         y_extrap = spline(x_extrap)
...
#         ax.plot(x, y, 'o', label='Original Data')
#         ax.plot(x_extrap, y_extrap, '--', label='Extrapolated Data')
#         ax.set_title('Signal Processing - Extrapolation Result')  # 영어로 설정
#         ax.legend()
#         st.pyplot(fig)
